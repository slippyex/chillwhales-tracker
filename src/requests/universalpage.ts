...
export async function fetchAssets(assetContract: string, gatherMode: GatherMode): Promise<Asset[]> {
    const url =
        config.universalPageCollectionBase +
        `/assets/${assetContract}/tokens?page=0&perPage=${config.amountToFetch}&sortBy=${gatherMode}`;
