import config, { universalPage } from '../config';
import axios from 'axios';
import { Asset, AssetHistory, GatherMode } from 'index';

export async function fetchFloorPricePer(skin: string): Promise<number> {
    try {
        const floorPriceUrl = universalPage.tokens + `?page=0&perPage=1&sortBy=price-low-high`;
...
export async function fetchAssets(gatherMode: GatherMode): Promise<Asset[]> {
    const url = universalPage.tokens + `?page=0&perPage=${config.amountToFetch}&sortBy=${gatherMode}`;
...
export async function fetchHistory(tokenId: string) {
    const url = universalPage.history + `/${tokenId}`;
    const response = await axios.get<AssetHistory[]>(url, {
        headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:98.0) Gecko/20100101 Firefox/98.0'
        }
    });
